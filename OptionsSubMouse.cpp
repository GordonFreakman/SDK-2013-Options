...
void COptionsSubMouse::OnControlModified(Panel *panel)
{
	...
	if (panel == m_pMouseAccelExponentSlider && m_pMouseAccelExponentSlider->HasBeenModified())
	{
		UpdateSensitivityLabel();
	}
...
void COptionsSubMouse::UpdateSensitivityLabel()
{
	...
	char buf2[64];
	Q_snprintf(buf2, sizeof(buf2), " %.1f", m_pMouseAccelExponentSlider->GetSliderValue());
	m_pMouseAccelExponentLabel->SetText(buf2);
}
