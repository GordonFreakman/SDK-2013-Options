...
public:
	...
	void MarkDefaultSettingsAsRecommended()
	{
		...
		SetComboItemAsRecommended( m_pD3D9Ex, 1);
	...
	virtual void ApplyChanges()
	{
		...
		ConVarRef mat_disable_d3d9ex("mat_disable_d3d9ex");
		if (m_pD3D9Ex->GetActiveItem() != mat_disable_d3d9ex.GetInt())
		{
			// notify the user that this will require a disconnect
			QueryBox *box = new QueryBox("#GameUI_D3D9Ex", "#GameUI_D3D9ExRelaunchMsg");
			box->AddActionSignalTarget(this);

			box->DoModal();
		}

		ApplyChangesToConVar("mat_disable_d3d9ex", m_pD3D9Ex->GetActiveItem());
	...
	{
		if (engine->IsInGame() == true)
		{
			if (engine->IsLevelMainMenuBackground() == false)
				return false;
...
COptionsSubVideo::COptionsSubVideo(vgui::Panel *parent) : PropertyPage(parent, NULL)
{
	...
	vgui::ComboBox *m_pVRMode = new ComboBox(this, "VRMode", 2, false);
	vgui::Label *m_pVRModeLabel = new Label(this, "VRModeLabel", "");
   ...
   m_pThirdPartyCredits->SetCommand(new KeyValues("OpenThirdPartyVideoCreditsDialog"));
	...
	m_pVRModeLabel->SetVisible(false);
